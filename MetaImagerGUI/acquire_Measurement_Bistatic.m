function [ g ]= acquire_Measurement_Bistatic(hObject,handles,vobj_switch,vobj_vna,axes1,F,num_switches,num_banks, buffersize, background)


Nf = length(F);
G = zeros(length(F)*num_switches*num_switches,1);
g = zeros(length(F)*num_switches*num_switches,1);
...
for sn1=1:num_switches %cycle through s1 switches
    ...
    for sn2=1:num_switches %cycle through s1 switches
