function [H F Az El Z] = measurement_matrix_build_two_Overlap_Bistatic(dmin,dmax,nfreqs,Azmin,Azmax,Elmin,Elmax,dataA,dataB)%filename)
...
H=zeros(101*6,nxy*nz);
for sn=1:6
        H2 = Eax(:,:,:,:,sn).*Ebx(:,:,:,:,sn)+Eay(:,:,:,:,sn).*Eby(:,:,:,:,sn);%+Eaz(:,:,:,:,sn).*Ebz(:,:,:,:,sn);
        for zn=1:nz
             for fn=1:length(F);
                Hzfs = H2(:,:,zn,fn);
                H(fn+length(F)*(sn-1),(1:nxy)+nxy*(zn-1)) = Hzfs(:);
             end
    end
end
